        ...
        with download_lock:
            ...
            for dl_id, dl_info in downloads.items():
                ...
                for k, v in dl_info.items():
                    if k in ("proc", "future", "subscribers", "stop_progress"):
                        continue
                    try:
...
def load_queue():
    ...
    try:
        ...
        for dl_id, dl_info in list(downloads.items()):
            ...
            if status in (STATUS_COMPLETED, "deleted"):
...
def download_worker(download_id: str, model_id: str, model_type: str,
    ...
    try:
        ...
        with download_lock:
            if download_id not in downloads:
                # Removed mid-flight (delete/clear): clean up partials and exit
        ...
        with download_lock:
            ...
            if current_status == STATUS_CANCELLED:
                ...
                downloads[download_id]["end_time"] = datetime.now().isoformat()
                if download_id in download_events:
                    download_events[download_id].put({"type": "error", "message": err_msg})
        ...
        with download_lock:
            if download_id in downloads:
                ...
                if stop_event:
                    stop_event.set()
        if download_id in download_events:
            download_events[download_id].put({"type": "error", "message": str(e)})
    finally:
        with download_lock:
            if download_id in downloads:
...
def list_downloads():
    with download_lock:
        ...
        for dl_info in downloads.values():
            ...
            for key in ("proc", "future", "subscribers", "stop_progress"):
