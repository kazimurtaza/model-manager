...
## Development
```bash
# Run the Flask dev server locally (auto-reload; note FLASK_DEBUG is ignored under the gunicorn image)
pip install -r requirements.txt
MODELS_PATH=./tmp-models DATA_PATH=./tmp-data FLASK_DEBUG=1 python app.py

# Or run the published image:
MODELS_HOST_PATH=/tank/models docker compose up
